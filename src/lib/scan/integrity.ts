...
export function diffIntegrity(
  ...
  // Guard against legacy baselines captured before integrity tracking existed.
  // Without a recorded script/egress baseline, EVERYTHING would look "new" and
  // fire false-positive CRITICALs. Skip until a baseline with integrity data is
  // re-established (the next baseline write stores scan.signals.scripts/egress).
  if (baseline.scripts === undefined && baseline.egress === undefined) {
    notes.push(
      "Integrity: no script/egress baseline recorded yet — supply-chain diff skipped until baseline is re-established.",
    );
